...
class PermissionUtil constructor(activity: FragmentActivity) {
    ...
    fun request(vararg permissions: String, callback: (areGrantedAll: Boolean, deniedPermissions: List<Permission>) -> Unit) {
        ...
        permissions.forEach { permission ->
            if (isGranted(permission)) {
                permissionsResult.add(Permission(permission, true, false))
            } else {
